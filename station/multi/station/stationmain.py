        ...
        while True:
            ...
            while backOff > 0:
                ...
                if time.time() - begin > slottime:
                    channel = 0
                    break
                time.sleep(1000 * (10**(-6)))    #  Time for sending the packet
                if time.time() - begin > slottime:
                    channel = 0
                    break
                pktrec[station_id] += 1
                channel = 0
print(3)
# excelind = int(input("Enter the index: "))
excelind = int(sys.argv[1])
# Number of Stations
...
while itr > 0:
    ...
    for i in range(len(grpsta)):
        ...
        print((sum(pktrec) * 256 * 8)/(notimes * 0.65 * 1024 * 1024 * (endtime - starttime)))
        print('')
    itr -= 1
    avg = (sum(pktrec) * 256 * 8)/(notimes * 0.65 * 1024 * 1024 * (endtime - starttime))
else:
    plt.title("Fairness")
    ...
    sheet = wb.active
    sheet.cell(row = excelind, column=2).value = avg
